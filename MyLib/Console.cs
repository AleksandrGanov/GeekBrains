...
namespace mlConsole
{
    ...
    public static class PrintData
    {
        ...
        /// <summary>
        /// Метод выводит на печать в консоль двумерный массив
        /// </summary>
        /// <typeparam name="T">Тип данных массива</typeparam>
        /// <param name="arr">Двумерный массив</param>
        /// <param name="cellWidth">Ширина "ячейки" для вывода элемента массив</param>
        /// <param name="mes">Подпись после вывода массива, после подписи выводится размерность масссив</param>
        public static void TwoDimArrayPrint<T>(T[,] arr, int cellWidth, string mes="Массив:")
        {
            int row = arr.GetLength(0), col = arr.GetLength(1);
            T[] oneDimArr = new T[arr.GetLength(1)];
            for (int j = 0; j < row; j++)
            {
                for (int i = 0; i < col; i++) oneDimArr[i] = arr[j, i];
                ArrPrint<T>(oneDimArr, oneDimArr.Length, cellWidth, false);
                Console.CursorTop -= 1;
            }
            Console.WriteLine();
            Console.WriteLine($"\n{mes} {col}x{row}");
